...
const decodePSSH = (data) => {
  ...
  console.log('\n', Buffer.from(psshData, 'base64').toString('ascii'))

  if (systemId === Buffer.from(system.PLAYREADY.id)) {
    // pro
  ...
  return {
    systemId: systemId.toString('hex'),
    data: Buffer.from(psshData, 'base64').toString('ascii'),
    version: psshVersion,
    keyCount: keyCountInt
  }
}
