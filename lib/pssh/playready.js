...
const constructProXML4 = (keyId, licenseUrl, keySeed) => {
  let key = generateContentKey(keyId, keySeed)

  let xmlData = `<WRMHEADER xmlns="http://schemas.microsoft.com/DRM/2007/03/PlayReadyHeader" version="4.0.0.0"><DATA><PROTECTINFO><KEYLEN>16</KEYLEN><ALGID>AESCTR</ALGID></PROTECTINFO><KID>${key.kid}</KID><CHECKSUM>${key.checksum}</CHECKSUM><LA_URL>${licenseUrl}</LA_URL></DATA></WRMHEADER>`
