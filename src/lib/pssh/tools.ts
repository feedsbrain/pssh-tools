  ...
  result.printPssh = () => {
    ...
    if (psshDataSize > 0) {
      ...
      if (systemName === system.WIDEVINE.name && result.dataObject) {
        const dataObject: T.WidevineData = result.dataObject as T.WidevineData
        if (dataObject.keyId) {
      ...
      if (systemName === system.PLAYREADY.name && result.dataObject) {
        const dataObject: T.PlayReadyData = result.dataObject as T.PlayReadyData
